...
describe('user Schema test anything', () => {
    ...
    it('Add User testing anything', async () => {
        const phonenumber = "9863841998"
        const new_password = "wang123"
        bcryptjs.hash(new_password, 10, function (err, hash) {
            var user = User.updateOne({phonenumber : phonenumber},{password : hash}).exec()
            return expect(user.password).toEqual(hash)
        })
